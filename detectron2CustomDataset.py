...
## En entrée : 
        # - instance_file_list : liste des chemins vers les masques d'instances ordonnés
        # - image_file_list : liste des chemins vers les images ordonnées
        # - instance_decode : fonction qui pour une instance donné, renvoie l'id de l'objet associé
## En sortie :
        # - dataset_dicts : dictionnaire regroupant toutes les informations sur toutes les instances de chaque masque d'instances
        # selon le format utilisable par detectron2   
    ...
    for instance_file,image_file in tqdm(zip(instance_file_list, image_file_list)):
        ...
        for label in np.unique(instance_img):
            ...
            if trainId !=255 and trainId!=0:
                ...
                #     cont = contours[0].tolist()
                #     cont = [y for xs in contours for x in xs for y in x]
                #     from detectron2.structures import BoxMode
                #     if(len(cont)>4):
                #         object_dict["bbox"] = [x,y,x+w,y+h]
                #         object_dict["bbox_mode"] = BoxMode.XYXY_ABS
                #         object_dict["segmentation"] = pycocotools.mask.encode(np.asarray(roi, order="F"))
                #         object_dict["category_id"] = trainId-1
                #         objects.append(object_dict)
...
## En entrée : 
        # - train : True si dataset d'entrainement, False si validation
        # - dir : chemin d'accès vers la dataset KITTI_seg, par défaut None et se réfère à la variable
...
def kitti_seg_instance(train,decoder,dir=None):
    ...
    if train:
        ...
        train_instance_file_list = [os.path.join(train_instance_dir,f) for f in os.listdir(train_instance_dir) if os.path.isfile(os.path.join(train_instance_dir,f))]
        train_instance_file_list.sort()
        train_image_file_list = [os.path.join(train_image_dir,f) for f in os.listdir(train_image_dir) if os.path.isfile(os.path.join(train_image_dir,f))]
        ...
        #Instanciation d'une liste de chemin vers vers les fichiers d'instances png
        val_instance_file_list = [os.path.join(val_instance_dir,f) for f in os.listdir(val_instance_dir) if os.path.isfile(os.path.join(val_instance_dir,f))]
        val_instance_file_list.sort()
        val_image_file_list = [os.path.join(val_image_dir,f) for f in os.listdir(val_image_dir) if os.path.isfile(os.path.join(val_image_dir,f))]
        val_image_file_list.sort()
        return create_dataset_dict(val_instance_file_list, val_image_file_list, decoder)

## En entrée : 
        # - instance_id : id de l'instance à décoder
## En sortie :
        # - trainId : valeur de l'id d'entrainement après décodage
