...
from mobile_utils import init_model_detectron2, init_model_monodepth2, init_model_pose, img2TensorPose, get_prediction, getQuantileId,bbox_IoU, sigmoid
from filter_model import mobileObjects_score
    ...
    with torch.no_grad():
        ...
        for sequence in sequence_files:
            ...
            if(len(image_files)>=2):
                ...
                mobject = mobileObjects_score(ratio_age=args.ratio, lost_age=args.lost_age+args.sort_age, 
                            dth=0.9, mth = args.mobile_score_th, min_age=args.filter_age)
                            ...
                            if len(object)==10:
                                ...
                                if len(dtrack)>0:
                                    ...
                                    if x1<=10 or x2>=img_width-11 or y1<=10 or y2>=img_height-11:
                                        mobile_score = sigmoid(args.sig1,(1-args.delta*quantile_mean_flow[idq,cy,cx]/max_mean_flow)-IoU)
                                        mobject.update(obj_id,score,mobile_score)
                                    else:
                                        mobile_score = (sigmoid(args.sig2,(diff-args.omega*quantile_std_flow[idq,cy,cx])/max_mean_flow) + 2*(sigmoid(args.sig3,1-IoU)-0.5))/2
                                        mobject.update(obj_id,score,mobile_score)
                ...
                output = sequence.replace("KITTI_raw", "KITTI_motion_score")
